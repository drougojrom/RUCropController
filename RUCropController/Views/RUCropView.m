...
typedef NS_ENUM(NSInteger, RUCropViewOverlayEdge) {
    RUCropViewOverlayEdgeNone,
    RUCropViewOverlayEdgeTopLeft,
    RUCropViewOverlayEdgeTop,
    RUCropViewOverlayEdgeTopRight,
    RUCropViewOverlayEdgeRight,
    RUCropViewOverlayEdgeBottomRight,
    RUCropViewOverlayEdgeBottom,
    RUCropViewOverlayEdgeBottomLeft,
    RUCropViewOverlayEdgeLeft
};

@interface RUCropView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) RUCropScrollView *scrollView;         /* The scroll view in charge of panning/zooming the image. */
...
@property (nonatomic, assign) RUCropViewOverlayEdge tappedEdge; /* The edge region that the user tapped on, to resize the cropping region */
...
/* Crop box handling */
- (RUCropViewOverlayEdge)cropEdgeForPoint:(CGPoint)point;
...
- (void)setup
{
    ...
    self.scrollView = [[RUCropScrollView alloc] initWithFrame:self.bounds];
...
- (void)updateCropBoxFrameWithGesturePoint:(CGPoint)point
{
    ...
    switch (self.tappedEdge) {
        case RUCropViewOverlayEdgeLeft:
            ...
            break;
        case RUCropViewOverlayEdgeRight:
            if (self.aspectRatioLockEnabled) {
                aspectHorizontal = YES;
            ...
            break;
        case RUCropViewOverlayEdgeBottom:
            if (self.aspectRatioLockEnabled) {
                aspectVertical = YES;
        ...
        case RUCropViewOverlayEdgeTop:
            ...
            break;
        case RUCropViewOverlayEdgeTopLeft:
            if (self.aspectRatioLockEnabled) {
                xDelta = MAX(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case RUCropViewOverlayEdgeTopRight:
            if (self.aspectRatioLockEnabled) {
                xDelta = MIN(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case RUCropViewOverlayEdgeBottomLeft:
            if (self.aspectRatioLockEnabled) {
                CGPoint distance;
                distance.x = 1.0f - (xDelta / CGRectGetWidth(originFrame));
                distance.y = 1.0f - (-yDelta / CGRectGetHeight(originFrame));
            ...
            break;
        case RUCropViewOverlayEdgeBottomRight:
            if (self.aspectRatioLockEnabled) {
            ...
            break;
        case RUCropViewOverlayEdgeNone: break;
    }
...
- (RUCropViewOverlayEdge)cropEdgeForPoint:(CGPoint)point
{
    ...
    if (CGRectContainsPoint(bottomLeftRect, point))
        return RUCropViewOverlayEdgeBottomLeft;
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated
{
    ...
    if (self.croppingStyle == RUCropViewCroppingStyleCircular) {
        delay = 0.0f;
    }
