...
#import "RUCropViewController.h"
#import "RUCropController-Swift.h"

static const CGFloat kRUCropViewControllerTitleTopPadding = 14.0f;

@interface RUCropViewController () <UIActionSheetDelegate, UIViewControllerTransitioningDelegate, RUCropViewDelegate>

/* The target image */
@property (nonatomic, readwrite) UIImage *image;

/* The cropping style of the crop view */
@property (nonatomic, assign, readwrite) RUCropViewCroppingStyle croppingStyle;

/* Views */
@property (nonatomic, strong) RUCropToolBar*toolbar;
@property (nonatomic, strong, readwrite) RUCropView*cropView;
...
@property (nonatomic, strong) RUCropViewControllerTransitioning *transitionController;
...
@implementation RUCropViewController

CGFloat titleLabelHeight;

- (instancetype)initWithCroppingStyle:(RUCropViewCroppingStyle)style image:(UIImage *)image
{
    ...
    if (self) {
        ...
        _transitionController = [[RUCropViewControllerTransitioning alloc] init];
        _image = image;
        _croppingStyle = style;
        
        _aspectRatioPreset = RUCropViewControllerAspectRatioPresetOriginal;
        _toolbarPosition = RUCropViewControllerToolbarPositionBottom;
...
{
    return [self initWithCroppingStyle:RUCropViewCroppingStyleDefault image:image];
}

- (void)viewDidLoad
{
    [super viewDidLoad];
    BOOL circularMode = (self.croppingStyle == RUCropViewCroppingStyleCircular);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.aspectRatioPreset != RUCropViewControllerAspectRatioPresetOriginal) {
...
- (CGRect)frameForToolBarWithVerticalLayout:(BOOL)verticalLayout
{
    ...
    if (!verticalLayout) {
        ...
        if (self.toolbarPosition == RUCropViewControllerToolbarPositionBottom) {
        ...
        if (self.toolbarPosition == RUCropViewControllerToolbarPositionTop && self.prefersStatusBarHidden == NO) {
...
- (CGRect)frameForCropViewWithVerticalLayout:(BOOL)verticalLayout
{
    ...
    if (!verticalLayout) {
        ...
        if (_toolbarPosition == RUCropViewControllerToolbarPositionBottom) {
            frame.origin.y = 0.0f;
...
- (CGRect)frameForTitleLabelWithSize:(CGSize)size verticalLayout:(BOOL)verticalLayout
{
    ...
    frame.origin.y = self.topLayoutGuide.length + kRUCropViewControllerTitleTopPadding;
...
- (void)adjustCropViewInsets
{
    ...
    verticalInset += kRUCropViewControllerTitleTopPadding;
...
- (void)viewDidLayoutSubviews
{
    ...
    [UIView performWithoutAnimation:^{
        self.toolbar.statusBarVisible = (self.toolbarPosition == RUCropViewControllerToolbarPositionTop && !self.prefersStatusBarHidden);
        self.toolbar.frame = [self frameForToolBarWithVerticalLayout:self.verticalLayout];
    ...
    if (NSClassFromString(@"UIAlertController")) {
        ...
        for (NSString *item in items) {
            UIAlertAction *action = [UIAlertAction actionWithTitle:item style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [self setAspectRatioPreset:(RUCropViewControllerAspectRatioPreset)i animated:YES];
...
{
    [self setAspectRatioPreset:(RUCropViewControllerAspectRatioPreset)buttonIndex animated:YES];
    self.aspectRatioLockEnabled = YES;
}
#pragma clang diagnostic pop

- (void)setAspectRatioPreset:(RUCropViewControllerAspectRatioPreset)aspectRatioPreset animated:(BOOL)animated
{
    ...
    switch (aspectRatioPreset) {
        case RUCropViewControllerAspectRatioPresetOriginal:
            aspectRatio = CGSizeZero;
            break;
        case RUCropViewControllerAspectRatioPresetSquare:
            aspectRatio = CGSizeMake(1.0f, 1.0f);
            break;
        case RUCropViewControllerAspectRatioPreset3x2:
            aspectRatio = CGSizeMake(3.0f, 2.0f);
            break;
        case RUCropViewControllerAspectRatioPreset5x3:
            aspectRatio = CGSizeMake(5.0f, 3.0f);
            break;
        case RUCropViewControllerAspectRatioPreset4x3:
            aspectRatio = CGSizeMake(4.0f, 3.0f);
            break;
        case RUCropViewControllerAspectRatioPreset5x4:
            aspectRatio = CGSizeMake(5.0f, 4.0f);
            break;
        case RUCropViewControllerAspectRatioPreset7x5:
            aspectRatio = CGSizeMake(7.0f, 5.0f);
            break;
        case RUCropViewControllerAspectRatioPreset16x9:
            aspectRatio = CGSizeMake(16.0f, 9.0f);
            break;
        case RUCropViewControllerAspectRatioPresetCustom:
            aspectRatio = self.customAspectRatio;
            break;
    }
    
    //If the image is a portrait shape, flip the aspect ratio to match
    if (aspectRatioPreset != RUCropViewControllerAspectRatioPresetCustom &&
...
- (void)cropViewDidBecomeResettable:(RUCropView *)cropView
{
    self.toolbar.resetButtonEnabled = YES;
}

- (void)cropViewDidBecomeNonResettable:(RUCropView *)cropView
{
...
- (id <UIViewControllerAnimatedTransitioning>)animationControllerForPresentedController:(UIViewController *)presented presentingController:(UIViewController *)presenting sourceController:(UIViewController *)source
{
    ...
    self.transitionController.prepareForTransitionHandler = ^{
        typeof (self) strongSelf = weakSelf;
        RUCropViewControllerTransitioning *transitioning = strongSelf.transitionController;
...
- (void)doneButtonTapped
{
    ...
    if (self.showActivitySheetOnDone) {
        RUActivityCroppedImageProvider *imageItem = [[RUActivityCroppedImageProvider alloc] initWithImage:self.image cropFrame:cropFrame angle:angle circular:(self.croppingStyle == RUCropViewCroppingStyleCircular)];
        RUCroppedImageAttributes *attributes = [[RUCroppedImageAttributes alloc] initWithCroppedFrame:cropFrame angle:angle originalImageSize:self.image.size];
...
#pragma clang diagnostic pop
    ...
    if (self.croppingStyle == RUCropViewCroppingStyleCircular && (isCircularImageDelegateAvailable || isCircularImageCallbackAvailable)) {
...
- (RUCropView *)cropView {
    ...
    if (!_cropView) {
        _cropView = [[RUCropView alloc] initWithCroppingStyle:self.croppingStyle image:self.image];
...
- (RUCropToolBar *)toolbar {
    if (!_toolbar) {
        _toolbar = [[RUCropToolBar alloc] initWithFrame:CGRectZero];
...
- (void)setCustomAspectRatio:(CGSize)customAspectRatio
{
    ...
    [self setAspectRatioPreset:RUCropViewControllerAspectRatioPresetCustom animated:NO];
}
